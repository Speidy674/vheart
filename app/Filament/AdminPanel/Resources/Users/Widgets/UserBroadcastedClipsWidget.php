...
class UserBroadcastedClipsWidget extends ChartWidget
{
    ...
    {
        $total = Clip::query()
            ->where('broadcaster_id', $this->record->getKey())
            ->count();

        return "$total Clips Broadcasted";
    }
    ...
    protected function getData(): array
    {
        ...
        $results = $this->executeQuery('clips', $start, $end, $interval);
    ...
    protected function getBaseQuery(string $table, CarbonInterface $start, CarbonInterface $end, string $interval): Collection
    {
        ...
        return DB::table(DB::raw(
            ...
            ->leftJoin($table, function ($join) use ($truncUnit, $table, $submitterId): void {
                ...
                )->where("$table.broadcaster_id", $submitterId);
            })
